...
public class WorkFormServiceImpl extends ServiceImpl<WorkFormMapper, WorkForm> implements WorkFormService {
    ...
    public void insertWorkForm(List<List<PassengerFlowExcelDto>> passengerFlowExcelVoLists, Map<String, String> ruleMap, double storeArea, String storeId) {
        ...
        //
        QueryWrapper<WorkForm> queryWrapper = new QueryWrapper<>();
        queryWrapper.between("date", workForms.get(0).get(0).getDate(), workForms.get(workForms.size() - 1).get(0).getDate());
        queryWrapper.eq("store_id", storeId);
        int flag = baseMapper.selectCount(queryWrapper);
        if (flag != 0) {
     ...
     */
    @Override
    public List<WorkForm> getDayShift(String storeId, LocalDate date, String position, String employeeName) {
        QueryWrapper<WorkForm> wrapper = new QueryWrapper<>();
        ...
        if (position != null && position != "") {
            ...
            wrapper.and(w -> {
                for (int i = 0; i < s.length; i++) {
                    String s1 = s[i];
                    w.like("allow_career", s1).or();
                }
                w.like("allow_career", "all");
                return w;
            });
    ...
    public List<User> getConformEmployee(ShiftVo shiftVo) {
        ...
        String[] position = shiftVo.getAllowCareer().split(",");
        for (int i = 0; i < userList.size(); i++) {
            User user = userList.get(i);
            boolean flag = false;
            for (String s : position) {
                if ("all".equals(s) || user.getCareer().equals(s)) {
                    flag = true;
                    break;
                }
            }
    ...
    //获取员工的某天工作时长和周工作时长
    @Override
    public User getEmployeeWorkHour(String employeeId, LocalDate date,LocalDate startDate,LocalDate endDate) {
        User user=new User();
        ...
        for(WorkForm workForm:workFormList){
            double shiftTime=TimeUtil.calculateStringTimeSub(workForm.getStartTime().toString(),workForm.getEndTime().toString());
            if(date.compareTo(workForm.getDate())==0){
                user.setDayWorkTime(user.getDayWorkTime()+shiftTime);
            }
            user.setWeekWorkTime(user.getWeekWorkTime()+shiftTime);
        }
